...
import frams
from helpers import eaSimple_modified2
...
def interpolate200(left, right, point, total=200):
    return point/total * (right-left) + left

def change_probs(gen):
    max_gen = 200
    # 0.01 - 1
    frams.GenMan.f1_smX = interpolate200(0.1, 0.05, gen)
    # 0.1 - 0.01
    frams.GenMan.f1_smJunct = interpolate200(0.05, 0.02, gen)
    # 0.1 - 0.02
    frams.GenMan.f1_smComma = interpolate200(0.05, 0.02, gen)
    # 0.1 - 1
    frams.GenMan.f1_smModif = interpolate200(0.2, 1, gen)

    # 1 - 0.01
    frams.GenMan.f1_nmNeu = interpolate200(0.1, 0.01, gen)
    # 0.5 - 0.1
    frams.GenMan.f1_nmConn = interpolate200(0.2, 0.1, gen)
    # 1 - 0.1
    frams.GenMan.f1_nmProp = interpolate200(0.1, 0.1, gen)
    # 0.01 - 1
    frams.GenMan.f1_nmWei = interpolate200(1, 1, gen)
    # 0.01 - 1
    frams.GenMan.f1_nmVal = interpolate200(0.5, 0.05, gen)


def frams_evaluate(frams_cli, individual, gen):
    change_probs(gen)
...
if __name__ == "__main__":
    ...
    pop, log = eaSimple_modified2(pop, toolbox, cxpb=parsed_args.pxov, mutpb=parsed_args.pmut,
