...
for method in methods:
    ...
    for series in range(1,11):
        path = f'logs/logs_{method}-{series}.pickle'
        ...
        plt.figure(1)
        plt.plot(data1, linewidth=0.5, c=color(method, 0.4), linestyle='-')
    ...
    err = data2['std']/3
    plt.figure(2)
    plt.plot(x, y, c=color(method, 1), linestyle='-')
    plt.fill_between(x, y-err, y+err, color=color(method, 0.3))
...
data3_fit = pd.DataFrame(
    index=range(1,11), columns=methods)
data3_time = pd.DataFrame(
    index=range(1,11), columns=methods)
for method in methods:
    for series in range(1,11):
        file_path = f'HoFs/HoF-vel-prawd{method}-{series}.gen'
        with open(file_path, 'r') as f:
            HoF = f.read().split('\n')
            f_val = float(next((row for row in HoF if row.startswith("velocity:")), 'velocity:0')[9:])
            time = float(HoF[-1][2:]) 
            data3_fit.at[series, method] = f_val
            data3_time.at[series, method] = time
