...
def save_tool(hof, clear):
    # print(f"saving..{parsed_args.series}")
...
def save_genotypes(filename, OPTIMIZATION_CRITERIA, hof, timer=None):
...
if __name__ == "__main__":
    ...
    if parsed_args.genformat == '1':
        sc = 0.00006 # similarity coefficient
    elif parsed_args.genformat == '4':
        sc = 0.0000009
    else: # 0, H
        sc = 0.00012
    similar = lambda x,y: x.fitness.values[0] - sc < y.fitness.values[0] and x.fitness.values[0] + sc > y.fitness.values[0]

    pop = toolbox.population(n=parsed_args.popsize)
    hof = tools.HallOfFame(parsed_args.hof_size, similar=similar)
