...
def frams_evaluate(frams_cli, individual):
	...
	try:
		...
		# print("data Type:", type(first_genotype_data))
		# print(first_genotype_data)
		body_recording = default_evaluation_data['data->bodyrecording']
		T = len(body_recording)
		assert T > 0
		body_recording = np.asarray(body_recording)

		# np.set_printoptions(precision=4, suppress=True)
		# with open(f'output\\recording\\test_output_{datetime.datetime.now().hour}-{datetime.datetime.now().minute}.txt', "w") as file:
		# 	file.write(genotype)
		# 	file.write("\n")
		# 	for l in body_recording:
		# 		file.write(str(l))
		# 		file.write("\n")

		### 1
		# fitness = [body_recording[:,:,1].max()]

		### 2
		# dist1 = 0
		# dist2 = 0
		# for i in body_recording[0, 1:]:
		# 	for j in body_recording[0, 2:]:
		# 		dist = np.linalg.norm(j-i)
		# 		dist1 = max(dist, dist1)
		# for i in body_recording[-1, 1:]:
		# 	for j in body_recording[-1, 2:]:
		# 		dist = np.linalg.norm(j-i)
		# 		dist2 = max(dist, dist2)
		# fitness = [dist2/dist1]

		### 3
		# print(body_recording.shape)
		p1 = body_recording[0].reshape((1,body_recording[0].shape[0],3))
		p2 = body_recording[0].reshape((body_recording[0].shape[0],1,3))	
		p3 = p1 - p2	
		# print(p1.shape, p2.shape, p3.shape)
		dist = np.linalg.norm(p3, axis=2)
		max_dist_start = dist.max()
		p1 = body_recording[-1].reshape((1,body_recording[-1].shape[0],3))
		p2 = body_recording[-1].reshape((body_recording[-1].shape[0],1,3))	
		p3 = p1 - p2
		dist = np.linalg.norm(p3, axis=2)
		max_dist_end = dist.max()

		fitness = [max_dist_end/max_dist_start]

		### 4 potencjalnie: min/max dist liczone po całej symulacji
...
def prepareToolboxFrams(frams_cli, tournament_size, genetic_format, initial_genotype):
...
if __name__ == "__main__":
	...
	toolbox_fram = prepareToolboxFrams(framsLib, parsed_args.tournament, 
						  '1' if parsed_args.genformat is None else parsed_args.genformat, 
						  parsed_args.initialgenotype)

	pop = toolbox_fram.population(n=parsed_args.popsize)
	...
	pop, log = algorithms.eaSimple(pop, toolbox_fram, cxpb=parsed_args.pxov, 
								mutpb=parsed_args.pmut, ngen=parsed_args.generations, 
	...
	save_genotypes(f"output/native/HoF_{parsed_args.series}.gen", OPTIMIZATION_CRITERIA, hof)
	file_path = f"output/native/log_{parsed_args.series}.pickle"
	pickle.dump(log, open(file_path,'wb'))
	# with open(f'output/native/hof_{parsed_args.series}.txt', 'w') as f:
	# 	ind = hof[0]
	# 	print(ind.fitness, file=f)
	# 	print(ind, file=f)
	# with open(f'output/native/log_{parsed_args.series}.txt', 'w') as f: 
	# 	print(log, file=f)
	with open(f'output/native/time_{parsed_args.series}.txt', 'w') as f: 
		print(measuresd_time, file=f)
