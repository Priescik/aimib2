...
for mode in MODES:
    for energy in ENERGIES:
        ...
        for series in SERIES:
            path = f'output/{mode}/e{energy}/log_e{energy}s{series}.pickle'
...
for mode in MODES:
    for energy in ENERGIES:
        for series in SERIES:
            with open(f'output/{mode}/e{energy}/HoF_e{energy}s{series}.gen', 'r') as f:
                HoF = f.read().split('\n')
                f_val = float(next((row for row in HoF if row.startswith("vertpos:")), 'vertpos:0')[8:])
                data3_fit.at[series, energy] = f_val
            with open(f'output/{mode}/e{energy}/time_e{energy}s{series}.txt', 'r') as f:
                time = f.readline()
                data4_time.at[series, energy] = float(time)
