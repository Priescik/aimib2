...
def prepareToolboxGP(frams_cli, tournament_size, genetic_format, initial_genotype):
...
if __name__ == "__main__":
	...
	toolboxGP = prepareToolboxGP(framsLib, parsed_args.tournament, 
						  '1' if parsed_args.genformat is None else parsed_args.genformat, 
						  parsed_args.initialgenotype)

	pop = toolboxGP.population(n=parsed_args.popsize)
	...
	pop, log = algorithms.eaSimple(pop, toolboxGP, cxpb=parsed_args.pxov, 
								mutpb=parsed_args.pmut, ngen=parsed_args.generations, 
								stats=stats, halloffame=hof, verbose=True ) #, 
								# hof_hist_path=parsed_args.hof_savefile + '_hist.txt')
	...
	save_genotypes(f"output/GP/HoF_{parsed_args.series}.gen", OPTIMIZATION_CRITERIA, hof)
	file_path = f"output/GP/log_{parsed_args.series}.pickle"
	pickle.dump(log, open(file_path,'wb'))
	# with open(f'output/GP/hof_{parsed_args.series}.txt', 'w') as f:
	# 	ind = hof[0]
	# 	print(ind.fitness, file=f)
	# 	print(gp.compile(ind, pset), file=f)
	# with open(f'output/GP/log_{parsed_args.series}.txt', 'w') as f: 
	# 	print(log, file=f)
	with open(f'output/GP/time_{parsed_args.series}.txt', 'w') as f: 
